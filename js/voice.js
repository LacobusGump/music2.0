 ...
 * VOICE — The Presence
 *
 * Pre-rendered ElevenLabs audio as primary voice.
 * Web Speech API as fallback for dynamic lines only (name, time context).
 */
...
const Voice = (function () {
  ...
  // Web Speech — only for lines that need the user's name or real-time context
  ...
  function lensSelected(name) {
    // Short lines — Web Speech fine here, these fire on user gesture
    var lines = {
  ...
  function onDiscovery() {
    // Intentionally empty — removed per design
  }
  ...
  function setName(raw) {
    ...
    awaitingName = false;
    // Dynamic: must use their actual name
  ...
  function onObservation(minutes) {
    ...
    // Scary — fires once, randomly, after 3+ minutes
    ...
    // Emotional — each line plays once, scattered through long sessions
    ...
    // Time-aware dynamic lines (Web Speech — real context)
