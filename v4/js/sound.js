...
const Sound = (function () {
  ...
  // v4 WAND — tremor drives "human life" in the voices (flutter, breath, jitter, noise)
  var _wandTremor = 0;
  // Public so index.html / conductor can push live values
  // (also exposed on the returned Sound object)
  ...
  function _synthLofiRhodes(time, freq, vel, decay) {
    ...
    var t = _wandTremor || 0;  // v4 — hand tremor makes the tape breathe and the vinyl speak
    ...
    var tremDepth = ctx.createGain(); tremDepth.gain.value = (0.09 + t * 0.13) * vel;  // tremor deepens the drowsy wobble
    trem.connect(tremDepth); tremDepth.connect(tremoloGain.gain);
    ...
    var flutterG = ctx.createGain(); flutterG.gain.value = freq * (0.0023 + t * 0.0021);  // more tremor = more cassette wow
    ...
    var noiseG = ctx.createGain(); noiseG.gain.value = (0.016 + t * 0.035) * vel;  // tremor brings the vinyl up — the room is in the groove
  ...
  function _synthSoulKeys(time, freq, vel, decay) {
    ...
    var t = _wandTremor || 0;  // v4 — tremor adds organic chorus width and keybed life
    ...
    var tremDepth = ctx.createGain(); tremDepth.gain.value = (0.07 + t * 0.09) * vel;  // tremor makes the chorus breathe like a real player
    trem.connect(tremDepth); tremDepth.connect(tremoloGain.gain);
  ...
  return Object.freeze({
    ...
    // v4 WAND + live environment
    setWandTremor: function(v) { _wandTremor = Math.max(0, Math.min(1, v || 0)); },
    get wandTremor() { return _wandTremor; },
    // setMicRoomMix / startMicInput wired from index.html (live env sampling)
  });
})();
