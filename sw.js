...
self.addEventListener('fetch',e=>{
  ...
  if(path.match(/\.(mp3|wav|ogg)$/)){
    e.respondWith(
      fetch(e.request).then(r=>{
        if(r.ok){const c=r.clone();caches.open(V).then(cache=>cache.put(e.request,c));}
        return r;
  ...
  // static assets: cache-first (JS, CSS, images — change rarely)
  if(path.match(/\.(js|css|png|jpg|jpeg|webp|ico|svg|woff2?)$/)){
    e.respondWith(
      caches.match(e.request).then(r=>r||fetch(e.request).then(res=>{
        if(res.ok){const c=res.clone();caches.open(V).then(cache=>cache.put(e.request,c));}
        return res;
      }))
    );
    return;
